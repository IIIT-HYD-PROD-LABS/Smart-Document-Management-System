...
**Current status (2026-05-21):** v1.0 SHIPPED · v2.0 Phases 9-13 CODE-COMPLETE · v2.0.1 patch in production · v2.1 shipped (per-tenant branding + BYOK AI + vendor-invoice rebrand) · v2.1.1 IA reset · **v2.1.2 production-readiness sweep shipped 2026-05-21**: agent-team audit closed cross-tenant IDOR in `view_email`, BYOK API-key reflection in `test_credential`, NUL-byte 500 in document search, slowapi `Response` parameter on all rate-limited AI routes, BYOK rate limits at 10 to 20 per minute, k8s-style health-check split (`/api/health/live` for docker, `/api/health` for monitoring), Next.js 15.5.18 backport (8H/4M/2L Dependabot advisories cleared), python-multipart 0.0.27 (last HIGH advisory), wildcard major-version ignore in `dependabot.yml` after 8 breaking-major PRs were closed · CI + Deploy GREEN on `main` · See [STATUS_REPORT.md](./docs/status/STATUS_REPORT.md) for the full session log and [SECURITY.md](./docs/security/SECURITY.md) for the per-finding table.
