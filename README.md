...
**Current status (2026-06-04):** Swappable LLM provider + notice-upload + perf + classification sweep, recorded in [GODMODE-AUDIT-2026-06-04.md](./docs/status/GODMODE-AUDIT-2026-06-04.md). The compliance notice/AI surface no longer hard-requires a per-tenant BYOK key: a new `resolve_credential` resolver falls back to a server-default provider driven by `LLM_PROVIDER` (Ollama runs locally with no key today; switch to OpenAI/Anthropic/Gemini later by changing one env var). Ollama and OpenAI adapters were added to the compliance provider factory; verified end to end against live Ollama `llama3.2`. Notice uploads now accept every type the generic uploader does (pdf, png, jpg, jpeg, tiff, bmp, docx), validated by extension plus magic bytes (was PDF/JPG/PNG only). Classification silent-failures fixed via a typed `ExtractionEngineError`: empty/failed extraction is no longer marked COMPLETED-with-UNKNOWN, OCR-engine and missing-model faults retry instead of returning empty, multi-page TIFF reads all frames, and scanned-PDF truncation is surfaced. Frontend: cross-client React Query cache contamination fixed (active client id added to review/search/AI-credential keys), documents list paginated, dashboard hover jank removed (CSS `:hover` instead of per-mousemove DOM mutation), rows/charts memoized; upload UI widened with an inline error state and reframed AI-availability copy. Verified green: about 705 backend tests pass (0 failed, ~32 skips) run in memory-bounded chunks against an ephemeral PG15, `tsc` clean, `next build` succeeds. To switch to GPT later: set `LLM_PROVIDER=openai` + `OPENAI_API_KEY`. Prior sweep below.

**Status (2026-05-26):** Full phase 1 to 18 end-to-end audit and hardening sweep, recorded in [GODMODE-AUDIT-2026-05-26.md](./docs/status/GODMODE-AUDIT-2026-05-26.md). A 14-agent parallel audit found and fixed roughly 30 issues spanning auth, document classification, the compliance notice lifecycle, alerts and calendar, Gmail ingestion, BYOK AI, and the frontend. Highlights: fixed a 500 on `POST /notices/{id}/assign`; notices are now classified, risk-scored, and deadline-alerted at intake on both the manual-create and Gmail-ingestion paths (previously only on the first move to under_review); added segregation-of-duties to the 4-stage approval chain (no self-approval, no single user clearing multiple stages); closed IDOR defense-in-depth gaps on notice transition, chain, bulk, bills, credentials, and filter rules; rejected cross-client mode on report endpoints; wired holiday-adjusted deadlines into scheduled alerts; and fixed a dead settings link in the AI extraction UI. Migration `0037` adds dedup constraints (documents, regulatory calendar) and drops the `cross_client_view` policy on encrypted `ai_credentials`. Verified green: 608 backend tests pass (0 failed, 32 intentional skips), ruff clean, all migrations apply to head, frontend type-checks and builds. Note: `FERNET_KEY` must be set in the runtime environment or MFA and PII-encryption endpoints return 500. Deferred items (escalation idempotency hardening, bill auto-ingestion feature, low-severity polish) are tracked in the audit doc.

**Status (2026-05-22):** v1.0 SHIPPED · v2.0 Phases 9-13 CODE-COMPLETE · v2.1 shipped · v2.1.1 IA reset · v2.1.2 production-readiness sweep · v2.1.3 compliance flow rewire (2026-05-21 PM) · **v2.1.4 admin shell + auth hardening shipped 2026-05-22**: unified `/dashboard/admin/` shell with secondary sidebar (Overview, Users, User detail, Early access, Audit log, AI provider, Organization, Security, Model eval). Auth hardening closed two CRITICAL findings: (1) `POST /api/auth/register` now requires a single-use early-access invitation JWT after the bootstrap admin (closing the public-register bypass that made the early-access gate decorative), (2) first-user admin promotion is wrapped in a Postgres advisory lock so concurrent register or OAuth callbacks cannot both insert as admin (closing the count-then-insert race at three call sites). Plus four HIGH/MED follow-ups: `accept-invite` now uses a typed `AcceptInviteRequest` schema, JWT invitation tokens removed from API responses, `/api/auth/oauth/diag` admin-gated outside DEBUG, and `is_active` filter added to the admin-promotion count. Compliance defense-in-depth: `GET /notices/{id}` and `GET /review/{id}` now apply an explicit `client_id` filter on top of RLS. Frontend perf: dashboard stats wrapped in React Query (60s stale), analytics dual-fetch wrapped (5min stale), recharts lazy-loaded via dynamic import, soft logout via `auth:session-expired` custom event removes the full-page reload on token refresh failure. 414 backend tests pass. **v2.1.3 compliance flow rewire shipped 2026-05-21 (PM)**: admin-only client creation, email-based team invite flow (pre-creates a pending User and emails a 7-day signed JWT, invitee sets password on `/accept-invite` and is signed-in), dedicated `POST /notices/{id}/assign` endpoint with a real-time WebSocket notification to the assignee (via the `notifications:{client_id}` Redis pubsub channel), the review queue is no longer empty by design: `rule_based_heuristic_v1` synthesises authority + type confidences from the ner extractor output today (BERT in v2.1 takes over automatically when it lands), and a new `POST /api/compliance/review/manual-enqueue/{notice_id}` endpoint lets any active member flag a notice they think the classifier got wrong. Compliance review page redesigned as a triage workbench: sticky filter strip with reason buckets, card grid with segmented confidence dots and a 75 percent threshold marker, Confirm / Re-classify / Open actions per card, manual-flag form embedded in the empty state. CI + Deploy GREEN on `main`. See [STATUS_REPORT.md](./docs/status/STATUS_REPORT.md) for the full session log and [SECURITY.md](./docs/security/SECURITY.md) for the per-finding table.
