...
def _stamp_extraction_fields(
    notice, envelope: dict, decision: dict, *, fill_columns: bool
) -> None:
    """Pure (no I/O) column-stamping logic shared by the sync/async twins.
...
def apply_extraction_to_notice(
    ...
    """Persist the extraction envelope onto the notice (sync — Session).

    Stays sync: app/tasks/document_tasks.py (Celery) calls this with a sync
    Session. Router callers use `apply_extraction_to_notice_async` instead.
    """
    _stamp_extraction_fields(notice, envelope, decision, fill_columns=fill_columns)
    db.flush()
...
async def apply_extraction_to_notice_async(
    db: AsyncSession, notice, envelope: dict, decision: dict, *, fill_columns: bool = True
) -> None:
    """Async twin of `apply_extraction_to_notice`, for the router call chain."""
    _stamp_extraction_fields(notice, envelope, decision, fill_columns=fill_columns)
    await db.flush()

    if decision.get("action") == "review_queue":
        await _enqueue_for_review_async(db, notice, envelope, decision)
...
async def _enqueue_for_review_async(db: AsyncSession, notice, envelope: dict, decision: dict) -> None:
    """Async twin of `_enqueue_for_review`.

    review_queue_service.enqueue_low_confidence is permanently sync (its own
    docstring: shared with the Celery ingestion pipeline, out of scope for
    the async migration), so it cannot accept an AsyncSession. This
    duplicates its small idempotent upsert here for the async router call
    chain rather than touching that file. Keep in sync with
    `enqueue_low_confidence` if its upsert shape ever changes.
    """
    from decimal import Decimal
    ...
    auth_low = authority_conf < CONFIDENCE_THRESHOLD
    type_low = predicted_type_confidence < CONFIDENCE_THRESHOLD
    if not (auth_low or type_low):
        return
    reason = derive_reason(authority_conf, predicted_type_confidence)
