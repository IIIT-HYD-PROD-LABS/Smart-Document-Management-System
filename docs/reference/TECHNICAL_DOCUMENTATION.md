# TaxSync — Final Product Technical Documentation

**Version:** v2.0.1
**Document date:** 8 May 2026
**Audience:** CA firms, finance teams, compliance leadership, technology evaluators, and prospective enterprise clients
...
**Production status.** The platform is production-ready: the document management product is mature and in everyday use; the compliance management layer (notices, deadlines, alerts, approval workflows, Gmail-driven intake, household bill tracking) is feature-complete. 64 user-facing features are live across 9 functional areas, backed by 389 backend automated tests, comprehensive security hardening, and continuous deployment infrastructure.
...
Each area is documented in detail in [Section 5: Feature Catalog](#5-feature-catalog).
...
Deadline alerts scheduled (T-7 / T-3 / T-1 / overdue) via durable scheduler
   │
   ▼
Multi-channel dispatch (Email + WebSocket; SMS scaffolded)
```
...
| E6 | Multi-channel reminder delivery | Email + in-app bell (SMS scaffolded for future activation) |
...
|---|---|---|
| Email | Production | SMTP via Resend (configurable to any SMTP provider) |
| In-App WebSocket | Production | Redis pub/sub channel `notifications:{client_id}`, FastAPI WebSocket forwards to authenticated subscribers |
| SMS | Scaffolded | Twilio adapter implemented; activation pending DLT registration and `users.phone` column |
| Browser Web Push | Roadmap | VAPID-keyed Web Push API with Service Worker |
...
For each recipient × channel:
   ...
   ├─ (Future) SMS → Twilio send → record provider message ID
   ├─ (Future) Push → VAPID Web Push → record subscription endpoint
   │
   ▼
...
### 9.5 Push Notifications (Roadmap)

Browser-native push notifications via the Web Push API are on the roadmap:

- Backend: VAPID key generation, `push_subscriptions` table, `WebPushSender` class plugged into the existing channel registry
- Frontend: Service Worker registration, subscription management, permission UX
- No third-party service dependency (VAPID is decentralized)
...
Critical configuration (full list in `backend/.env.example`):
...
### 17.1 Available Now (v2.0.1)

64 features live across 9 functional areas, listed in detail in [Section 5: Feature Catalog](#5-feature-catalog).

### 17.2 v2.1 (Next Release — Q3 2026)

| Feature | Notes |
|---|---|
| Higher-accuracy AI classification across 40+ notice sub-types | BERT-based replacement for the rule-based classifier |
| AI-drafted response generation from notice templates | LLM-assisted first draft for response workflow |
| 20+ pre-built response templates | Variable substitution with notice-specific fields |
| Evidence package PDF merge with auto table-of-contents | One-click submission package |
| GSTR-2A/2B vs GSTR-3B reconciliation report | Automated input tax credit reconciliation |
| Searchable regulation library | GST Act, IT Act, Companies Act, FEMA, SEBI regulations indexed for in-app reference |
| Larger-scale search infrastructure | Elasticsearch / Elastic Cloud for very high notice volumes |
| SMS alerts (production) | Pending DLT registration with Indian telecom regulator |
| Per-user alert preferences | T-7 / T-3 / T-1 customization per user |
| Calendar export | Google Calendar / Outlook (.ics) for statutory deadlines |
| Severity-weighted compliance score | Refinement of the rolling 90-day health score |
| Outlook, Yahoo, IMAP email integrations | Beyond Gmail |
| Browser Web Push notifications | VAPID-based push for closed-tab alerts |

### 17.3 v3.0 (Future — 2027)

| Feature | Notes |
|---|---|
| In-app AI chat | "Ask AI about my notices" conversational interface |
| Outbound email replies via AI drafting | Send responses directly from the platform |

### 17.4 Blocked (External Dependency)

| Feature | Blocker |
|---|---|
| Direct integration with GST, Income Tax, MCA portals (auto-fetch notices) | Pending regulatory access decisions and GST Suvidha Provider (GSP) empanelment |

### 17.5 Out of Scope

| Feature | Reason |
|---|---|
| Native mobile apps (iOS / Android) | Web-first sufficient for target users |
| WhatsApp Business notifications | Email + SMS sufficient |
| Multi-language interface (Hindi, regional) | English standard for compliance documentation |
...
| **GSP** | GST Suvidha Provider. An authorized intermediary that connects software to the GST portal. |
...
| Field | Value |
|---|---|
| Document title | TaxSync — Final Product Technical Documentation |
| Product version | v2.0.1 |
| Document date | 8 May 2026 |
| Document audience | CA firms, finance teams, compliance leadership, technology evaluators, prospective enterprise clients |
| Sections | 18 |
| Features documented | 64 production features |
| Source documents | Live source code, automated test suite, deployment configuration, security audit history |

---

*TaxSync — Smart Document Management and Compliance System for Indian Regulatory Practice*
